<template lang="html">
  <div class="post-preview">
    <router-link :to="{name:'Post', params: {subPermalink: post.sub.permalink, id: post.id}}" class="image-area">
    ...
    <router-link :to="{name:'Post', params: {subPermalink: post.sub.permalink, id: post.id}}" class="body-area">
      <h3><slot></slot></h3>
      <p>

        <span>on {{ post.created }}</span>
        <span v-show="!hideUser">&nbsp;by <span class="blue-highlight">u/{{ post.user.username }}</span></span>
        <span v-show="!hideSub">&nbsp;in <span class="blue-highlight">s/{{ post.sub.name }}</span></span>
        </p>
      <p>{{ description }}</p>
    </router-link>
  </div>
