...
def posts_create(username: str):
    schema = Schema({
        ...
        "sub": And(str, len, error="Sub not specified"),
        "content": And(str, len, error="Content not specified"),
    })
    validated = schema.validate(dict(request.form))

    sub_permalink = validated["sub"]
    sub = Sub.objects(permalink__iexact=sub_permalink).first()
    if not sub:
        return jsonify({"error": f"Sub '{sub_permalink}' not found"}), 404
